...
export const HandoutTray: React.FC<HandoutTrayProps> = ({ isOpen, handouts, onClose, onCreate, onEdit, onShare, onPreview }) => {
  ...
  const handleCreateTrigger = (handout: Handout) => {
    ...
    show({ type: 'info', message: t('vtt.handouts.tray.triggerTool.notification').replace('{name}', handout.name) });
    onClose();
  };
        ...
        <div className="p-3 border-b border-zinc-800 space-y-3">
          <div className="relative">
            ...
            <input
              ...
              placeholder={t('vtt.handouts.tray.searchPlaceholder')}
              value={search}
          ...
          <button onClick={onCreate} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-sm font-bold bg-primary/10 border border-primary/30 rounded-lg text-primary hover:bg-primary hover:text-white transition-all shadow-sm">
            <Plus className="w-4 h-4" /> {t('vtt.handouts.tray.newResource.button')}
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
          {filteredHandouts.length === 0 ? (
            <div className="text-center p-8 text-zinc-600 italic text-sm">
              {search ? t('vtt.handouts.tray.noResults.text') : t('vtt.handouts.tray.noResources.text')}
            </div>
          ) : (
              ...
              return (
                <div key={handout.id} className="bg-zinc-900/50 border border-zinc-800 rounded-lg p-2 group transition-all hover:border-zinc-600 hover:bg-zinc-900">
                  ...
                  <div className="flex justify-between gap-1 mt-2 pt-2 border-t border-zinc-800/50">
                    <button onClick={() => handleCreateTrigger(handout)} className="px-2 py-1 text-[10px] font-bold rounded flex items-center gap-1 text-purple-400 hover:bg-purple-900/20 transition-colors" title={t('vtt.handouts.tray.createTrigger.title')}>
                      <Zap className="w-3 h-3" /> {t('vtt.handouts.tray.trigger.button')}
                    </button>
                    <div className="flex gap-1">
                      <button onClick={() => onEdit(handout)} className="p-1.5 text-zinc-500 hover:text-white hover:bg-zinc-800 rounded transition-colors" title={t('vtt.handouts.tray.edit.tooltip')}>
                        <Edit className="w-3.5 h-3.5" />
                      </button>
                      <button onClick={() => onShare(handout)} className={`p-1.5 rounded transition-colors ${sharedCount > 0 ? 'text-primary bg-primary/10' : 'text-zinc-500 hover:text-white hover:bg-zinc-800'}`} title={t('vtt.handouts.tray.share.tooltip')}>
