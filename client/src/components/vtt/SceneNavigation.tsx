...
export const SceneNavigation: React.FC = () => {
    ...
    const { scenes, activeSceneId, switchScene, addScene, deleteScene, updateSceneData } = useGameSession();
    const { openModal, closeModal } = useModal();
    const [isOpen, setIsOpen] = useState(false);
    const [isEditing, setIsEditing] = useState<string | null>(null);
    const [editName, setEditName] = useState('');

    // We strictly use AccessGate for global guarding to ensure only GMs see this tool
    // but AccessGate generally renders "something" unless mode is hide.
    // If we want it HIDDEN for players, we use mode="hide".
    // We wrap the entire return.
