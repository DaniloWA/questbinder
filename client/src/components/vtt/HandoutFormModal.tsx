...
export const HandoutFormModal: React.FC<HandoutFormModalProps> = ({ handout, onSave, onClose }) => {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (response.success && response.data) {
            ...
            show({ type: 'error', message: response.message || t('vtt.handouts.formModal.uploadError.text') });
        }
    };
        ...
        return (
            <div className="flex flex-col h-[70vh] max-h-[800px]">
                <div className="text-center mb-6">
                    <h2 className="text-2xl font-bold font-fantasy text-white">{t('vtt.handouts.formModal.chooseTemplate.title')}</h2>
                    <p className="text-zinc-400 text-sm">{t('vtt.handouts.formModal.chooseTemplate.subtitle')}</p>
                </div>
        ...
        <div className="flex flex-col h-[85vh] max-h-[900px]">
            ...
            <div className="flex flex-col gap-4 mb-4 shrink-0 bg-zinc-900/30 p-3 rounded-lg border border-zinc-800/50">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-1">
                        <SheetInput
                            label={t('vtt.handouts.formModal.title.label')}
                            value={name}
                            onChange={(e) => setName(e.target.value)}
                            placeholder={t('vtt.handouts.formModal.title.placeholder')}
                            variant="box"
                            className="bg-zinc-950 border-zinc-700 font-bold text-lg"
                        />
                    </div>
                    <div className="w-full md:w-48">
                        <SheetLabel>{t('vtt.handouts.formModal.theme.label')}</SheetLabel>
                        <div className="relative">
                            ...
                            >
                                <option value="standard">{t('vtt.handouts.formModal.theme.standard')}</option>
                                <option value="parchment">{t('vtt.handouts.formModal.theme.parchment')}</option>
                                <option value="terminal">{t('vtt.handouts.formModal.theme.terminal')}</option>
                                <option value="arcane">{t('vtt.handouts.formModal.theme.arcane')}</option>
                            </select>
                ...
                <div className="flex items-center gap-4">
                    <div className="flex bg-zinc-950 p-1 rounded-lg border border-zinc-700">
                        <button type="button" onClick={() => setType('text')} className={`px-3 py-1.5 text-xs font-bold rounded flex items-center gap-2 transition-all ${type === 'text' ? 'bg-zinc-700 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}><FileText className="w-3.5 h-3.5" /> {t('vtt.handouts.formModal.type.text')}</button>
                        <button type="button" onClick={() => setType('image')} className={`px-3 py-1.5 text-xs font-bold rounded flex items-center gap-2 transition-all ${type === 'image' ? 'bg-zinc-700 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}><Image className="w-3.5 h-3.5" /> {t('vtt.handouts.formModal.type.image')}</button>
                        <button type="button" onClick={() => setType('video_link')} className={`px-3 py-1.5 text-xs font-bold rounded flex items-center gap-2 transition-all ${type === 'video_link' ? 'bg-zinc-700 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}><Youtube className="w-3.5 h-3.5" /> {t('vtt.handouts.formModal.type.video')}</button>
                    </div>
                        ...
                        {type === 'text' ? (
                            <>
                                ...
                                <SheetTextArea
                                    ...
                                    placeholder={t('vtt.handouts.formModal.content.placeholder')}
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center p-8 bg-zinc-900/30">
                                <div className="w-full max-w-lg space-y-4">
                                    <div className="text-center text-zinc-500 mb-4">
                                        ...
                                        <p className="text-sm">{t('vtt.handouts.formModal.urlOrUpload.text')}</p>
                                    </div>
            ...
            <div className="flex justify-between gap-3 pt-4 border-t border-zinc-800 mt-4 shrink-0">
                <Button variant="ghost" onClick={() => setStep('template')}>
                    <LayoutTemplate className="w-4 h-4 mr-2" /> {t('vtt.handouts.formModal.changeTemplate.button')}
                </Button>
                <div className="flex gap-3">
                    <Button variant="ghost" onClick={onClose}>{t('common.actions.cancel.label')}</Button>
                    <Button onClick={handleSave} disabled={!name.trim() || isUploading} className="min-w-[120px]">
                        {handout ? t('vtt.handouts.formModal.saveChanges.button') : t('vtt.handouts.formModal.createResource.button')}
                    </Button>
                </div>
            </div>
        </div>
