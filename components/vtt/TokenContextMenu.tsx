    ...
    >
      ...
      <div className="flex flex-col gap-1 mb-1">
        ...
        {canEdit && (
            ...
            >
                <button className="flex items-center justify-between w-full text-left px-2 py-2 text-sm text-zinc-300 rounded-md hover:bg-white/10 hover:text-white transition-colors bg-zinc-800/50">
                    <div className="flex items-center gap-3">
                        <Activity className="w-4 h-4" /> Condições
                    </div>
                    <ChevronRight className="w-3 h-3 opacity-50" />
                </button>

                {showConditionSubmenu && (
                    <div 
                        className="absolute left-full top-0 ml-2 w-64 bg-zinc-900/95 border border-zinc-700 rounded-xl shadow-xl p-1.5 max-h-80 overflow-y-auto custom-scrollbar z-[10000]"
                        onMouseEnter={handleConditionMouseEnter} // Garante que o mouse enter no filho mantenha aberto
                        onMouseLeave={handleConditionMouseLeave}
                    >
                        {/* Ponte invisível estendida para cobrir o gap e evitar fechamento acidental */}
                        <div className="absolute right-full top-0 h-full w-6 bg-transparent -mr-2" />
                        
                        <div className="px-2 py-1 mb-1 text-[10px] font-bold uppercase text-zinc-500 tracking-wider sticky top-0 bg-zinc-900/95 backdrop-blur z-10 border-b border-zinc-800/50">Status & Efeitos</div>
                        {Object.entries(STATUS_RULES).map(([key, rule]) => {
                            const isActive = liveToken.conditions?.includes(key);
                            return (
                                <button 
                                    key={key}
                                    onClick={(e) => { e.stopPropagation(); handleConditionClick(key); }}
                                    className={`w-full text-left px-2 py-1.5 text-xs rounded flex items-center justify-between gap-2 mb-0.5 transition-colors ${isActive ? 'bg-primary/20 text-white font-medium' : 'text-zinc-300 hover:bg-zinc-800'}`}
                                >
                                    <span>{rule.name}</span>
                                    {isActive && <div className="w-2 h-2 bg-primary rounded-full shadow-[0_0_5px_rgba(124,58,237,0.8)]"></div>}
                                </button>
                            );
                        })}
                    </div>
